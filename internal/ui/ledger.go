...
func (m *Model) cycleLaunder() {
	...
	m.say(fmt.Sprintf("Launder dial %s: washing up to %s/day, audit risk %s/day. %s", d, money(m.set.Laundering.Capacity(m.w)), format.Pct(m.set.Laundering.AnyAuditRisk(m.w), 1), launderBlurb(d)))
}
