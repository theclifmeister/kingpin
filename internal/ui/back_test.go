...
func TestBackIsOneKey(t *testing.T) {
	...
	dialogs := []dialog{
		...
		{"front", modeFront, func(m *Model) { m.Update(key("7")); m.Update(key("b")); m.Update(key("down")) }, func(m *Model) int { return m.front.step }, func(m *Model) int { return m.front.kind }, 1, []string{"enter"}},
...
func TestTabIsSilentWhereThereIsNoPage(t *testing.T) {
	cases := []struct {
		...
		{"cash out", modeCashOut, func(m *Model) { m.w.Player.CleanCash = 200_000; m.Update(key("7")); m.Update(key("c")) }},
		{"report", modeReport, func(m *Model) { m.mode = modeReport }},
		{"help", modeHelp, func(m *Model) { m.Update(key("?")) }},
		{"card outcome", modeCard, func(m *Model) {
