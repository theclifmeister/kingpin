...
func TestHelpScrollsToTheLastRow(t *testing.T) {
	...
	if len(words) != 62 {
		t.Errorf("WORDS has %d terms, want the dial, the float, the till (#459), the target, heat, the rungs (patrol, sting, raid, arrest), the file, pressure, goodwill, cover, estimate (#355), drift, undercut, keep at, through, standing, connect, credit, unlock, house, deed, the pane, the strip, the tier, scout, boost, scene, world, quality, cut, cook, repeat, overdose, jailed, kin, driver, trait, captain (#346), lieutenant, temper (#455), asset, the feds, intel, spy, ending, score, quiet day, run out, absorbed, scattered, gone (#472), walk away, reign, favour, war, character, daily, profile and preset", len(words))
	}
	m.Update(key("?"))
	if m.mode != modeHelp {
