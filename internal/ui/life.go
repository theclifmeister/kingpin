...
func (m *Model) confirmDriver() {
	...
	m.say(fmt.Sprintf("%s drives %s from tomorrow: risk −%s a day on the road, and jailed if a shipment is seized.", who.Name, r.Name, format.Pct(m.set.Logistics.DriverCut(who.Skill), 0)))
}
...
func (m *Model) viewDriver() string {
	...
	for _, c := range rows {
		...
		var skill, cut any = c.Skill, "−" + format.Pct(m.set.Logistics.DriverCut(c.Skill), 0)
		switch {
		case c.ID == 0:
...
func (m *Model) driverLine(route string) string {
	...
	return fmt.Sprintf("%s · risk −%s", c.Name, format.Pct(m.set.Logistics.DriverCut(c.Skill), 0))
}
