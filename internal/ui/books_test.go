...
func TestBooksKeys(t *testing.T) {
	...
	m.Update(key("1"))
	m.Update(key("t"))
	if m.mode != modePlay || m.status != "Tip police on the map screen (5). Export order on the ledger screen (7). Buy trophy on the ledger screen (7)." { // t is tip (#241): cut is %, assign l; the ledger's export order (#391) and trophy (#392)
