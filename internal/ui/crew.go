...
func (m *Model) confirmInvestigate() {
	...
	m.say(fmt.Sprintf("Questions get asked tonight. Odds of a name ~%.0f%%.", m.rules.Crew.InvestigateOdds(m.w)*100))
}
...
func (m *Model) payOffConfirm() string {
	...
	body := []string{
		fmt.Sprintf("%s for %s: loyalty %.0f → %.0f.", money(m.rules.Crew.PayoffCost(*c)), c.Name, c.Loyalty, min(100, c.Loyalty+m.rules.Crew.PayoffLoyalty())),
...
func (m *Model) personLines(c game.CrewMember, onPayroll bool) []string {
	...
	lines = append(lines, keyRow("$", fmt.Sprintf("pay off for %s: %.0f → %.0f", money(m.rules.Crew.PayoffCost(c)), c.Loyalty, min(100, c.Loyalty+m.rules.Crew.PayoffLoyalty()))))
...
func (m *Model) askAroundRow() string {
	...
	return keyRow("i", fmt.Sprintf("ask around %s, names ~%.0f%%", money(m.rules.Crew.InvestigateCost()), m.rules.Crew.InvestigateOdds(m.w)*100))
}
