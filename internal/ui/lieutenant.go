...
func (m *Model) confirmAssign() {
	...
	m.say(fmt.Sprintf("%s runs %s from tonight: posts the idle crew, sells the stash, keeps %.0f%%.", lt.Name, m.w.CityName(city), m.rules.Crew.Cut()*100))
}
...
func (m *Model) viewAssign() string {
	...
	return m.pickerModal("ASSIGN "+lt.Name, nil, []col{{"city", kText, 0}, {"corners", kInt, 0}, {"units", kInt, 0}, {"runs", kText, 0}}, cells, m.pick.cursor,
		...
		theme.Subtle.Render(fmt.Sprintf("the stash at their dial; your own order wins. Cut %.0f%%, +%d crew slots.", m.rules.Crew.Cut()*100, m.cfg.Crew.Role[game.RoleLieutenant].Crew)),
