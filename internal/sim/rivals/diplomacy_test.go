...
func TestBetrayalFloorAndDistrust(t *testing.T) {
	...
	for w.Day < betrayed+dip.DistrustDays+40 && accepted == 0 {
		...
		if c := s.Chance(w, w.Rival(), *w.Today.Proposal); (w.Day+1-betrayed < dip.DistrustDays) != (c == 0) {
...
func TestChanceFollowsTheTable(t *testing.T) {
	...
	base := s.Chance(w, w.Rival(), truce(dip.TruceDays[1]))
	if base <= 0 || base >= 1 {
		t.Fatalf("base chance %.2f", base)
	}
	if s.Chance(w, w.Rival(), truce(dip.TruceDays[0])) <= base || s.Chance(w, w.Rival(), truce(dip.TruceDays[2])) >= base {
		t.Fatalf("truce length: short %.2f, standard %.2f, long %.2f", s.Chance(w, w.Rival(), truce(dip.TruceDays[0])), base, s.Chance(w, w.Rival(), truce(dip.TruceDays[2])))
	}
	w.Rival().Trust += 30
	if s.Chance(w, w.Rival(), truce(dip.TruceDays[1])) <= base {
	...
	fat := game.Deal{Kind: game.DealTribute, Terms: game.Terms{PerDay: s.Cut(w, w.Rival(), dip.TributeCuts[2])}}
	thin := game.Deal{Kind: game.DealTribute, Terms: game.Terms{PerDay: s.Cut(w, w.Rival(), dip.TributeCuts[0])}}
	if s.Chance(w, w.Rival(), fat) <= s.Chance(w, w.Rival(), thin) {
		t.Fatalf("tribute: fat %.2f, thin %.2f", s.Chance(w, w.Rival(), fat), s.Chance(w, w.Rival(), thin))
	}
	lines := w.SplitLines()
	...
	if s.Chance(w, w.Rival(), modest) <= 0 || s.Chance(w, w.Rival(), greedy) != 0 {
		t.Fatalf("split: modest %.2f, greedy %.2f", s.Chance(w, w.Rival(), modest), s.Chance(w, w.Rival(), greedy))
	}
	theirs := game.Deal{Kind: game.DealSplit, Terms: game.Terms{Corners: append(append([]string(nil), lines[0]...), "railyard")}}
	if s.Chance(w, w.Rival(), theirs) != 0 {
		t.Fatal("a split asking for a corner it holds was not refused")
	}
	if err := w.SendEnforcers("railyard", events.ForcePush); err != nil {
		t.Fatal(err)
	}
	if s.Chance(w, w.Rival(), truce(dip.TruceDays[1])) != 0 {
