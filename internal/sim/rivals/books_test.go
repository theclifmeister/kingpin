...
func TestBuyOffSendsHeadsHome(t *testing.T) {
	...
	price := s.MusclePrice(w, w.Rival())
	want := p.MusclePrice*s.CornerDay(w, w.Rival()) + p.CashShare*float64(w.Rival().Cash)/2
	if price != int(math.Round(want)) {
		t.Fatalf("price %d, want %.0f", price, want)
	}
	w.Player.Reputation.Respect = 100
	if got := s.MusclePrice(w, w.Rival()); got != int(math.Round(want*(1-cfg.Reputation.Effects.PoachPriceCut))) || got >= price {
	...
	if w.Rival().Away != 2 || w.Rival().AwayDay != w.Day || s.Want(w, w.Rival()) != max(0, int(math.Round(cfg.Rivals.Personality["defensive"].MusclePerCorner*2))-2) {
		t.Fatalf("away %d since day %d, wants %d", w.Rival().Away, w.Rival().AwayDay, s.Want(w, w.Rival()))
	}
