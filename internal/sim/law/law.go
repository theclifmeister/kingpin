...
func (s *Sim) Step(w *game.World, t *game.Tick) {
	...
	// winner on the sitting DA's ticket is the sitting DA re-elected. A
	// law-and-order DA elected on a loud enough city wants a new chief.
	if next := s.NextElection(w); next > 0 && t.Day >= next {
		mean := w.MeanPressure()
		law := s.LawAndOrderShare(mean) * (1 - tun.Moderate)
		reform := (1 - s.LawAndOrderShare(mean)) * (1 - tun.Moderate)
		rng := t.Sub(s.Name())
		r := rng.Float64()
		stance := "moderate"
		switch {
...
func (s *Sim) replaceChief(w *game.World, t *game.Tick, why string) {
	...
	w.Law.Chief = game.Chief{Name: name, Personality: content.ChiefPersonalities[rng.IntN(len(content.ChiefPersonalities))], Since: t.Day}
	w.Stats.Chiefs++
