...
func TestSaveMigratesTheStashIntoAStarterHouse(t *testing.T) {
	...
	got, err := Load(1, Migration{From: 11, Apply: MigrateHouses}, Migration{From: 12, Apply: func(w *World) { w.MigrateLots(50, 1) }}, Migration{From: 13, Apply: func(*World) {}}, Migration{From: 14, Apply: (*World).SeatRival})
	if err != nil {
		t.Fatal(err)
	}
	if len(got.Houses) != 1 {
