...
func TestSaveMigratesTheFallGuy(t *testing.T) {
	...
	for _, used := range []bool{true, false} {
		...
		got, err := Load(1, Migration{From: 9, Apply: MigrateFallGuys}, Migration{From: 10, Apply: func(*World) {}}, Migration{From: 11, Apply: MigrateHouses}, Migration{From: 12, Apply: func(w *World) { w.MigrateLots(50, 1) }}, Migration{From: 13, Apply: func(*World) {}}, Migration{From: 14, Apply: (*World).SeatRival}, Migration{From: 15, Apply: MigrateBooks})
		if err != nil {
			t.Fatal(err)
		}
		want := 0
