...
The game is a set of independent, deterministic simulations stepped once per in-game day over a shared `World`, with a Bubble Tea UI on top. `engine.Session` wires them. These are the invariants; break one only with an issue that says so.
...
**Events are the only cross-sim channel** (`docs/events.md`). Sims never call each other; a sim emits typed events (`internal/events`, each with a stable `Kind()`) via `Tick.Emit`, later sims in the same tick read `Tick.Events()`, and after the tick the bus delivers them to the UI. A new event kind goes in `events.All` (`TestAllListsEveryKind`) and gets a headline template in `content/headlines.toml` or a line in the news test's `reportOnly` (`TestEveryEmittedEventHasTemplate`, #274). Each sim folds its own tuning (`content.*Config`, the tree's `game.FoldEffects`) and never reads another sim's.
...
| Engine (#293) | `engine` | | `docs/engine.md` | `TestOneAssemblyPath` |
