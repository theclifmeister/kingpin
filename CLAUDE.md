...
`cmd/balance` policies: `idle | hide | quiet | normal | aggressive | careful | managed | upgraded | crewed | territory | war | diplomat | laundered | funded | corrupt | distributor | driven | delegated | dealer | stocked | routine | leveraged | boss | pricewar | saboteur | tipster | cook | retiree | cartel | reckless | informed` (an unknown one falls back to `normal`). What each plays, its flags (`-lielow -corners -force -rival -factions -cash -own -cards -incidents -lt -chief -da -undercut -credit -houses -fronts -heat -pace -cut -margin -life`) and what the output lines mean are in `docs/harness.md`. Day counts (`harness.Horizon`, `TierDays`, `-days`) are where the tooling *looks*, never a run length: the game has no day cap and a run ends only through an ending (#27). Do not add mechanics that end a run for playing on.
...
| Heat, evidence, the police response | `sim/heat`, levels `content.Patrol..Arrest` (the task force between the raid and the arrest, #48) | `heat.toml` | `docs/corners.md`, `docs/snitching.md`, `docs/assets.md` | `heat_test.go`, `taskforce_test.go`, `TestRichHiderIsNeverIndicted` (#27), `balance_test.go` |
...
| Property: deeds, rent, the forfeiture (#194) | `game/territory.go` (`BuyDeed`, `Deed`), `sim/territory` (rent, robbery), `sim/rivals` (`OddsOn`), `sim/heat` (`RaidWeight`), `sim/law` (pressure, the forfeiture), `ui/deeds.go` | `city.toml [deed]` | `docs/property.md` | `TestDeedsPullTheirWay` ×4, `TestNoDeedIsTheOldRun`, `TestDeedSlowsTheRivalNeverStopsIt`, `TestForfeiture`, `TestDeedSizing` |
| Assets: the connect, the port, the airstrip, the lab, the tunnel; the task force; tier 5 (#48) | `game/assets.go`, `sim/laundering` (the books), `sim/heat` (the rung), `sim/market`, `sim/logistics`, `sim/crew` (the effects), `ui/assets.go`, `harness/assets.go` | `assets.toml`, `heat.toml [[response]] taskforce`, `routes.toml` (`asset`), `progression.toml` tier 5 | `docs/assets.md` | `TestTaskForceNeverMeetsTierThree`, `TestNoAssetIsTheOldRun`, `TestCartelIsWithinFifteenPercentOfBoss`, `TestAggressiveCartelLosesItsAssets` |
| Crew life: kin, ageing, arrests, bail, shot, the driver (#46) | `sim/crew/life.go`, `Heat.Sweep`, `Bail` / `SetRouteDriver` in `game/actions.go`, `ui/life.go` | `crew.toml [life]`, `[role.driver]` | `docs/crew-life.md` | `life_test.go` ×3, `TestNoLifeIsTheOldRun`, `TestWarHasBodies`, `TestDriverCutsSeizures` |
...
| Factions: the table, arrivals, contests, absorption, alliance, poaching, arrest, homage, `Dominant` (#43) | `game/factions.go`, `sim/rivals/factions.go`, `sim/crew/factions.go`, `ui/rivals.go`, `theme.Factions` | `rivals.toml [factions]`, `law.toml [pressure] factions` | `docs/rival.md` | `TestOneFactionIsTheOldRun`, `TestFactionsContestBeforeDay120`, `TestNobodyIsDominantByAccident`, `factions_test.go` |
...
- The table is the duel's dice plus side streams (#43): faction 1 rolls on `Tick.RNG`, every other seat and everything factions do to each other on a `Sub`, so `harness.OneFaction` is byte-for-byte the old run; the table shares the map (`shared_pace`, `table_share`) or the bands break.
- Incidents are weather, not difficulty: the harness boxes the table (`harness.Play` deals it), an effect writes only state a sim already reads (`game.ApplyIncident`), and one that needs another sim's dice rides the `Incident` event.
