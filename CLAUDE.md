...
`cmd/balance` policies: `idle | quiet | normal | aggressive | careful | managed | crewed | territory`. An unknown policy silently falls back to `normal`. `-lielow N` overrides the lie-low threshold of the managed-style policies; `-corners N` caps how many corners `territory` works (`crewed` works as many as it can staff).
