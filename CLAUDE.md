...
The game is a set of independent, deterministic simulations stepped once per in-game day over a shared `World`, with a Bubble Tea UI on top. These are the invariants; break one only with an issue that says so.
