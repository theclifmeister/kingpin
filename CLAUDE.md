...
| Endings: nine causes and their owners, the exit plans, the score, the summary (#49) | `game/exit.go` (`End`, `Score`, `Retire`, `Vanish`), the owning sims, `ui/summary.go`, `ui/exit.go` | `endings.toml`; thresholds in the owners' | `docs/endings.md` | `TestEveryEndingIsReachable`, `TestExitPlansRunInOrder`, `TestEndingFrequencies`, `TestNoEndingIsTheOldRun` |
...
| Assets: the connect, port, airstrip, lab, tunnel; the task force; tier 5 (#48) | `game/assets.go`, `sim/laundering` (the books), `sim/heat` (the rung), the sims the effects touch, `ui/assets.go`, `harness/assets.go` | `assets.toml`, `heat.toml` taskforce, `routes.toml` (`asset`), `progression.toml` tier 5 | `docs/assets.md` | `TestTaskForceNeverMeetsTierThree`, `TestNoAssetIsTheOldRun`, `TestCartelIsWithinFifteenPercentOfBoss`, `TestAggressiveCartelLosesItsAssets` |
...
- Products above the first three are gated by `unlock_cash`; designer is the port's and reaches home only by road. A cash gate cannot separate tier 2 from tier 3; the ladder does.
...
- Incidents are weather, not difficulty: the harness boxes the table (`harness.Play` deals it), an effect writes only state a sim already reads (`game.ApplyIncident`), one that needs another sim's dice rides the `Incident` event.
