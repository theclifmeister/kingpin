...
`cmd/balance` policies: `idle | hide | quiet | normal | aggressive | careful | managed | upgraded | crewed | territory | war | diplomat | laundered | funded | corrupt | distributor | driven | delegated | dealer | stocked | routine | leveraged | boss | pricewar | saboteur | tipster | cook | retiree | cartel | reckless` (an unknown one falls back to `normal`). What each plays, its flags (`-lielow -corners -force -rival -factions -cash -own -cards -incidents -lt -chief -da -undercut -credit -houses -fronts -heat -pace -cut -margin -life`) and what the output lines mean are in `docs/harness.md`. Day counts (`harness.Horizon`, `TierDays`, `-days`) are where the tooling *looks*, never a run length: the game has no day cap and a run ends only through an ending (#27). Do not add mechanics that end a run for playing on.
...
**World is a plain struct** (`docs/world.md`) that sims mutate directly; player actions queue intent that the sims resolve (or, for a buy and a trip, apply at once), and the morning `DayReport` is what the UI shows. Stock moves only through `World.AddStock` / `TakeStock` and the house helpers (`TestStashHasNoWriters`, #144; a lot carries a quality the accessors keep, #47); the per-day scratch is one struct, `World.Today`, that the clock zeroes as a unit (`ClearToday`); a sim writes only its own state (`TestSimsWriteOnlyTheirOwnState`, the exceptions listed there), and `TestSeedDigest` pins sixty days of the boss on one seed so a moved number names its day. `SchemaVersion` is 15; bump it when `World` changes shape incompatibly and add a `game.Migration` for the previous version to `sim.Set.Migrations` (`docs/saves.md`). Zero values are the pre-feature state wherever they can be, so most features need no bump.
