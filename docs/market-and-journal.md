...
- **The categories** run in the order money moves through a night, and `game.FlowLabel` is the words for each (`Road, hires, fees` for `routes` and `Investments, tribute` for `investments` since #465: a night's signing fee with no route run read as the road's, and homage received as a return on an investment; the categories and their sums did not move):
  ...
  - `laundering`: the wash (dirty out, clean in), the upkeep of the fronts and the assets, what the fronts earn, the offshore account's fee;
  ...
  - `offshore`: clean cash moved into the offshore account (#422, appended last, so an old flow reads it as zero). It was part of `laundering`, and a night that put $50K offshore headlined "The night made -$34K" while the street made +$23K; the money is still yours.
...
- `corner_lost` (10): corners of yours gone back to the street (`CornerLost`, owner player), taken by a rival (`CornerTaken` from player) or walked off with a lieutenant (`LieutenantWalked`). Act: the map on the first corner. A night that leaves you no corner in a city adds, per city, `That was your last corner in Eastside: nothing sells there. Post on a free corner, push a rival's or buy a block on the map, or sell in Bayport.` (#471, `lastCorner`, read off the world the night left, no dice; `TestLastCornerSaysTheWaysBack`).
...
- `flow` (5 times the swing): the night's profit against the average of the `week` (7) nights before it in `World.Flows`, as a share of that average, from `min_swing` (0.5) and capped at `swing_cap` (1.5). Profit is the net less purchases, investments and what went offshore (#351's categories, `offshore` since #422): stock bought and money put in your own account are not money lost, and a buying day would read as a crash (`TestLeadIsTheBiggestThree`'s "money put offshore is not the night's loss"). It says nothing before there is a week. Words, a night against a night (#502: `on the week: +$4,656 against +$2,716 a night` read as a week against a night): `Profit fell 62% from the week's nightly average: +$9,500 last night against +$25K`, `Profit ran ×3.1 the week's nightly average: ...`, or `Last night made -$486 against the week's +$26K a night` when it went negative. Act: the ledger, where FLOW is. A fall at Territory with no route on, after a week that made no more than the one before it, adds the road (#446, `roadHint`): `The corners here have a ceiling: the road to Bayport is on the map (5).`, and the act is the map. The fortnight's test is `game.Plateau` over `CashFlow.Profit` (the lead's profit, shared since #476), and the first night the hint is said is stamped in `World.Progression.Ceiling` (the news sim's; 0 never, so an old save is the run before; no sim reads it, and `TestSeedDigest` leaves it out of its walk, `unwalked`): the engine's `port` alert reads it and stops a fast-forward once, where the plateau itself flickers week to week (`docs/cities.md`). `cmd/balance` over 120 days has every policy that stays home flattening to $1.5–4K a night after day 70 while the ones that run the road to the second city do not (`TestFallingProfitPointsAtTheRoad`: the hint at Territory on a flat fortnight, none with a route on, at an earlier tier or on a rising fortnight, and the ceiling stamped the first night only).
...
- `front_shut` (8): fronts shut for upkeep the clean pile could not pay (#458, `FrontFrozen`), one line for all of them: `Laundromat shut 7 days: its upkeep is paid in clean cash, and the clean pile was $150 short.`, `2 fronts shut 7 days: upkeep is paid in clean cash, and the clean pile was $550 short (Laundromat, Car Wash).` A playtest bought its first front with no clean cash and read only the flavour headline (`Sign on the door of Eastside Laundromat: closed until further notice`) and `shut, back in 2d`. Act: the ledger.
...
`reportLines` opens with the fast-forward's stop line, then TODAY in the title's gold and the lines numbered `1`, `2`, `3` in the key colour, each wrapped under itself to the modal's width rather than cut (#446: the end of a line is where the road's pointer rides), then the sections.
