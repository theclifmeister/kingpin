...
- **The versions.** `SUPPORTED` is `{protocol: [4], view: [7]}` (view 3 since #345, protocol 4 since #356, view 4 since #358, view 7 since #352). `checkVersions` refuses a module whose `kingpin.protocol` or `kingpin.view` is another (`VersionError`, shown full-page) before a run starts: a field renamed under the client would draw a wrong game instead of failing. A version bump in the engine fails `TestWebClient` until `SUPPORTED` moves with the client.
