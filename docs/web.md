...
- `js/session.js`: the protocol as the client speaks it. `Session` wraps one `kingpin.open()`: `call(method, ...params)` sends a request line and keeps the `event` notifications (`take()` hands them over) and the last `view`. It throws `RPCError` (`refused` for `-32000`, the game's words) on an error. The moves the page makes are one method each: `newRun`, `endDay`, `fastForward`, `choose`, `travel`, `buy`, `maxBuy`, `restockPlan`, `presets`, `presetDiff`, `applyPreset`, `preview` (#353), `sell`, `exportSave`, `importSave`. A `no_room` refusal (`NO_ROOM`, `-32002`, #356) is `refused` too, with `free`, what fits. `streetConnect(view, city)` is the open street connect where you stand. It touches no DOM.
- **The versions.** `SUPPORTED` is `{protocol: [21], view: [14]}` (view 3 since #345, protocol 4 since #356, view 4 since #358, view 5 since #351, view 6 since #355, view 7 since #352, protocol 5 since #357, view 8 since #343, protocol 6 since #344: `rules.territory.deed_rent` takes a price, a rule the client does not call; protocol 7 since #353; view 9 and protocol 8 since #346; protocol 9 since #341; protocol 10 and view 10 since #347: the client words the plan alert and reads neither the ambitions panel nor the pin; view 11 since #354; protocol 11 since #391: `set_export`, a command the client does not call; protocol 12 since #392: `buy_trophy` and `trophy_offers`, likewise; protocol 13 since #395: `cash_out`, likewise; protocol 14 since #398: `go_straight`, likewise, and the client words the `straight` alert; protocol 15 since #397: `forecast`, protocol 16 since #455: `rules.crew.lieutenancy`, and the client words the `exposure` alert; protocol 17 since #474: the `characters` query and the refusals, which the client meets nowhere (it starts the default character and offers no trip to the city you stand in); protocol 18 since #459: `rules.logistics.idle`, which the client does not call, and the client words the `till` alert, and since #475 the `arrest` alert (a warrant out), the `heat` alert by the rung its `level` names and the law panel's arrest rung as a warrant, with no version moved; protocol 19 since #472: `rules.rivals.down`, which the client does not call; protocol 20 since #478: `set_sweep` and `stop_sweep`, which the client does not call, and the ambitions' `dirty` unit, which `exits.js` formats as money; protocol 21 since #496 and #503: `set_till`, which the client does not call, and the client words the `landed` alert (fields alerts already had) and the till alert's raise; view 12 since #405: `exports` and `trophies`, which the client does not draw, and since #405 the client calls `cash_out`, `reserve`, `forecast`, `retire`, `vanish`, `crown` and `go_straight` from its money and walk-away panels; view 13 since #458: the client words the `front_shut` alert off its `front`; view 14 since #476: the client words the `port` alert off its `share`, and since #505 the `exports` alert (fields alerts already had) with no version moved). `checkVersions` refuses a module whose `kingpin.protocol` or `kingpin.view` is another (`VersionError`, shown full-page) before a run starts: a field renamed under the client would draw a wrong game instead of failing. A version bump in the engine fails `TestWebClient` until `SUPPORTED` moves with the client.
- `js/alerts.js` (#352): the alerts in words. `WORDS` is one sentence a kind (`plan` since #347, named from the view's `ambitions`), off the alert's fields and the view (a number the view left out is zero), `alertText(v, a)` the alert's; `PANELS` maps the engine's screens the page has onto its elements (the market onto the product table, the map onto the canvas) and `alertPanel(a)` is the element that answers an alert, or none. It touches no DOM.
...
- `js/lieutenants.js` (#455): a lieutenant in words, pure functions both web clients share (Street Edition copies it): `roleLines(terms)` is the role in four sentences off `rules.crew.lieutenancy` (the night's work; the cut and the crew slots; the four tempers and the days before theirs shows; the loyalty where they talk and where they walk with the city), `temperLine(temper)` one temper (`sells aggressive · heat ×1.25 · 3d stock · hits scouts`, the TUI pane's words), `temperOf`, `tempers` and `howTheyCome(view)`, the hint while fewer than two cities hold a corner of yours. The page's LIEUTENANTS section draws them and gives each lieutenant on the payroll a button a city (`session.assign`) and `Stand down` (`session.unassign`); `play.mjs` checks the words carry the terms' numbers and that `assign` reaches the engine.
- `js/exits.js` (#405): the ways out and the tonight line. `EXITS` is the four endings the player claims (retire, vanish, take the crown, go straight) in the TUI's walk-away order, each with the ambition that plans it and the session method that takes it; `exits(view)` reads each open (its ambition done, #347, whose steps are the ending's own terms) or with what is short in the steps' own labels and units (`the account $0 of $750,000; quiet days 0 of 14`), `claim(session, id)` takes one and returns the view over, and `tonightText(forecast)` is #397's forecast in words (`Tonight the police count $7.4M dirty ($2.5M now + $5.0M landing − $495 wages, before sales): $4.9M past your cover, +25 heat before the wash.`). The page's **Money** panel carries the tonight line (amber past the cover), a cash-out field with the fee the rule quotes (`rules.laundering.cash_out_fee`) and an offshore reserve field; its **Walk away** panel is a button a way out, greyed with what is short, asking before it ends the run. Before #405 the page had no way out at all, so since #227 and #398 the crown and the businessman were out of its reach.
...
- `js/main.js`: the page. It loads the module, opens a session and starts a run (the URL's `?seed=`, else a random one).
  ...
  - The panel is the city you stand in. Each product shows the street price, the connect's price and what you hold, with a buy quantity and buy and sell buttons at the chosen dial. Below is the law (#355): the police risk where you stand, a line each for the heat and the next rung, every rung with what it takes, the file and what fills it, the pressure and the goodwill, the dirty pile against the exposure line with the fronts' cover, and the cop's word as an estimate or `No word from inside`, what is close in red. Below that are travel, the pool looking for work (the view's `pool`, #332: each with a hire button, off when the fee is more than your dirty cash) and the morning report: TODAY first, the view's `report.lead` (#354), each line a button to the panel that answers it where the page has one (`alertPanel`, as an alert's), then the sections in the engine's one order, `report.sections`, the TUI's. The money section is the night's cash flow (#351, `report.flow`, `flowTable`): the opening, a row a category that moved, dirty, clean and both, the big ones in green or red, and the closing; a night that moved nothing falls back to the money lines.
  ...
  - End day opens the day's preview first (#353, `showPreview`, the `preview` query): the morning's alerts in their words (`alertText`; the idle crew and the corners nobody works among them), each a button that closes the preview and brings its panel into view where the page has one (`alertPanel`, #352), each city's sales tonight, tonight's money as the flow table (`flowTable` from `Now` to `~Closing`) and what the estimate leaves out (`UNKNOWN`). `End day` (`space` or `enter`) ends it, `Not yet` (`esc`) goes back. The next 7 days and the autopilot do not stop for it.
...
- `TestWebClient` builds the site as the command does and runs `testdata/play.mjs` under Node. The script loads the page's own modules with the engine loaded the way the page loads it, minus the DOM.
  ...
  - The ways out (#405): the Go test writes four saves on seed 7, each with one way out open (the account and the quiet days, the new identity, the reign, the businessman's streak); the page's `exits` reads that one open and the rest closed with what is short, `claim` ends the run on it (retired, vanished, kingpin, businessman), and on the first a cash-out of $1,000 takes $1,000 clean and lands it dirty less the quoted fee and the tonight line reads. A closed way out is refused by the engine, not the page.
  ...
  - The day's preview (#353) of a sale queued on day 0 carries the sale and every category, adds up from the opening to the closing, carries the view's alerts each with an act, and changes nothing.
  ...
  - Every alert of every played morning and every boss night is worded with nothing missing in it and carries an act, `WORDS` is `engine.AlertKinds`, no more and no less, and some alert over the runs links to a panel (#352). Every lead line has words and an act, and the report's sections are the fifteen in order (#354).
...
## Street Edition, the illustrated frontend (#383)

`frontends/street-edition/` (named Ink & Ambition until #409) is a separate illustrated HTML/CSS/ES-module interface on the same WASM engine. It preserves this reference client and the TUI. Build with `python3 frontends/street-edition/build.py`; serve its generated `dist/` on a static host. Its README lists prerequisites, controls and current UI gaps. It speaks protocol 19 since #472 and view 14 since #476 (13 since #458), its build copying the reference client's `session.js` and `alerts.js` (16 since #455, 15 since #407): the export lanes and their orders, the trophies, the cash-out, the tonight forecast and what each closed ending still needs.

The builder compiles `cmd/kingpin-wasm` from this checkout, copies the matching Go loader and the reference client's session/alert/police/lieutenant helpers, and derives role/trait descriptions from content TOML. A protocol/view guard requires an explicit frontend review when the contract moves. No compiled engine, duplicate simulation, hosted-project metadata or player saves are checked in.

The UI reads structured dilemma consequences, report sections/leading stories/cash flow, the day preview, police ladder and investigations, ambition steps, preset diffs, buy capacity, crew traits and captaincy, and the lieutenants (#455: a city to run through `assign` / `unassign`, the role's terms off `rules.crew.lieutenancy` worded by the shared `lieutenants.js`). Every change is an engine command. Local saves use `export_save`/`import_save`. The illustrated background is decorative; corner markers show engine state. `node frontends/street-edition/smoke.mjs` after building checks a real 45-day WASM run, query purity of the public view, structured choices, cash-flow reconciliation and save round-trip. The development validation records desktop/mobile and older-save checks.
