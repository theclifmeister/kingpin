...
| Endings: nine causes and their owners, the exit plans, the reign (#227) and its grace (#399), going straight (#398), the score, the summary (#49) | `game/exit.go` (`End`, `Score`, `Retire`, `Vanish`, `Crown`), the owning sims, `ui/summary.go`, `ui/exit.go` | `endings.toml`; thresholds in the owners' | `docs/endings.md` | `TestEveryEndingIsReachable`, `TestExitPlansRunInOrder`, `TestEndingFrequencies`, `TestNoEndingIsTheOldRun`, `TestGoingStraightIsClaimed`, `TestGoStraightFromTheDialog`, `TestReignBreaks`, `TestReignStopsOnce` |
