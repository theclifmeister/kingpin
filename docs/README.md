...
| Heat, evidence, the police response, targeted investigations (#343) | `sim/heat` (`investigation.go`), levels `content.Patrol..Arrest` (the task force before the arrest, #48), `game/investigation.go`, `engine/alerts.go` (`investigation`), `ui/alerts.go`, `harness/surgeon.go` | `heat.toml` (`[investigation]`) | `docs/corners.md`, `docs/snitching.md`, `docs/assets.md` | `heat_test.go`, `investigation_test.go` ×2, `taskforce_test.go`, `TestRichHiderIsNeverIndicted` (#27), `TestNoInvestigationIsTheOldRun`, `TestSurgeonBeatsManaged` |
...
| Dilemma cards | `game/dilemmas.go`, `sim/news/dilemmas.go`, `ui/card.go` | `dilemmas.toml` | `docs/dilemmas.md` | `dilemmas_test.go`, `TestTriggersHold`, `TestEverySlotIsFilledByItsTrigger`, `TestPersonalStakesAreCapped`, `TestRichCardsMoveMoreThanCash`, `TestRichDeckLeansToTheBand` |
