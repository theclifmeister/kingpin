...
| Factions: the table, arrivals (late seats, #389), contests, absorption and stranding, alliance, poaching, homage, succession, `Dominant` (#43) | `game/factions.go`, `sim/rivals/factions.go`, `sim/crew/factions.go`, `ui/rivals.go` | `rivals.toml [factions]` (`strand_days`, `succession_muscle`), `law.toml [pressure] factions` | `docs/rival.md` | `TestOneFactionIsTheOldRun`, `TestFactionsContestBeforeDay120`, `TestNobodyIsDominantByAccident`, `TestBrokeRoutedFactionScatters`, `TestBrokeRaidedFactionScatters`, `TestLateSeatSettlesOnWorkedGround`, `TestSeatWithNoRoomStandsDown`, `TestRichLandlessFactionResolves`, `factions_test.go` |
...
| Dashboard, map, ledger, rivals screens; one ▸ a screen (#462: `unfocused` in `ui/table.go`) | `ui/dashboard.go`, `map.go`, `routes.go`, `ledger.go`, `rivals.go`, `diplomacy.go` | | `docs/frame-and-pane.md`, `docs/ui.md` | `dashboard_test.go`, `TestRouteMarkerMoves`, `TestTablesAreConsistent`, `TestMapRouteInPane`, `TestMarketHasOneCursor`, `TestRivalsHaveOneCursor` |
