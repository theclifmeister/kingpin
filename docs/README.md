...
| Fast-forward, alerts, stop events, the alert jump (#352), a shortfall and a repeated offer stop once (#469), the last corner gone (#471) | `engine/stops.go` (`news`, `memo`, `OfferQuiet`), `alerts.go` (`Act`, `ActsOf`, `noCorners`), `ui/fast.go`, `ui/alerts.go` (`openAlert`) | `houses.toml` (`full_share`) | `docs/ui.md`, `docs/engine.md` | `TestFastForwardIsTheSameDays`, `TestFastForwardStopsOnACard`, `TestEveryAlertHasAnAct`, `TestUnpostedAlerts`, `TestStashFullAlerts`, `TestEveryAlertHasATarget`, `TestAlertJumpOpensItsTarget`, `TestEveryActLands`, `TestShortfallStopsOnce`, `TestRepeatedOfferRunsPast`, `TestLostCornersStop`, `TestLastCornerAlert`, `TestNoCornerAlertWords`, `TestWagesAlertSaysTheAccountIsOut`, `TestGoneRivalAndRetireStopSayWhatHappened` |
| The cart, the dialogs, the delta; where a dialog opens (#462); the modal's wrap (#463) | `ui/cart.go` (`cartTable`), `ui/dialogs.go` (`productInView`), `ui/market.go` `priceFacts`, `ui/modal.go` (`wrapLine`, `modalFollow`) | | `docs/cart.md`, `docs/modals.md` | `cart_test.go`, `delta_test.go`, `toggle_test.go`, `TestDialogOpensWhereItSays`, `wrap_test.go` (`TestWrapLine`, `TestModalsNeverCutProse`) |
