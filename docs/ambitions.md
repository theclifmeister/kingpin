...
- **The panel** (`modeAmbitions`, `ui/ambitions.go`): `a ambitions` on the dashboard (#472: the dashboard's own key, in help and the README), on the walk-away dialog's first page (`w` on the dashboard) and on the stage modal, where it marks the stage seen and closing the panel goes on to the card and the report.
  ...
  Its `home corners` step is the corners held against the share's line (`3 of 6`). Since #478 the walk-away dialog's closed crown row counts the crews still in the wings apart from those on the street (`3 of 6 corners held, 1 crew still standing, 2 crews yet to arrive`, `ui.crownShort` over `rivals.Sim.Down`): a seat yet to arrive blocks `Dominant()` as a crew standing does, and was the playtest's hidden clock (`TestCrownSaysWhoIsYetToCome`).
...
- **The vanish plan is the tree's whole chain** (#478): it read `retainer` and `identity` and left out the lawyer on call the retainer requires, so a player with $261K clean read 53% with the first node unbought. The step is paid in dirty cash, so it reads against the dirty pile (`UnitDirty`, the web clients format it as money); a plan done is still exactly `World.CanVanish` (`TestAmbitionProgressAgreesWithTheEnding`, `TestVanishPlanStartsWithTheLawyer`).
