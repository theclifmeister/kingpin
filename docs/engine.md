...
- **Alerts** (`engine/alerts.go`). `Session.Alerts()` returns `[]Alert`, loudest first, in the order of the `AlertKind` constants: `talking`, `contract_due`, `debt_due`, `heat` (where you are, at or over the patrol line), `task_force`, `float` (only while `FloatMatters`: a front, or a route with its dial on), `wages`, `crew_line`, `skim`, `unposted`, `idle_corner`, `stash_full`, `gate`, `house_known`, `da_race`, `retire`, `favour`, `reign` and `plan` (#347: the pinned plan's steps met in order, `docs/ambitions.md`).
  ...
  - **Every alert carries its act** (#352): `Alert.Act` is `{screen, mode, subject}`, what answers it. The engine names it and each front end maps it onto its own screens and dialogs; nothing new is simulated.
    ...
    `AlertKinds()` lists every kind in order and `ActsOf(kind)` every act a kind can carry, the usual one first: the talking, the wages and the skim the crew screen; a member near a line their row there; a contract due its buyer and a debt due its connect on the market; the heat, the task force, the reign and the plan the dashboard; the float, the DA race and the favour the ledger; a known house that house and a full stash that city's first house on the ledger; an idle corner the post picker on it; an unposted member the post picker on their corner, else their row; a gate the market for a product or a connect and the ledger for a front or an asset; retirement the ledger until it is open, then the dashboard's walk away.
...
- **The schema.** `protocol.Schema()` generates a JSON Schema document (draft 2020-12) from the Go types: the protocol and view versions, the framing, the error codes, every method's `params` (`prefixItems`, a dial as its enum of names) and `result`, the two notifications, every event kind's payload under `events`, and 224 named types under `$defs`. It is checked in as `internal/protocol/schema.json` (about 260 KB). `TestSchemaIsCurrent` fails when the file is stale, and `go test ./internal/protocol -run TestSchemaIsCurrent -update` rewrites it. `protocol.Version` (2 since the quotes, #325; 3 since the saves as bytes, #327; 4 since `max_buy`, `restock_plan` and `no_room`, #356; 5 since the presets, #357; 7 since `pin_ambition`, #347, with 6 queued ahead of it) moves with the methods; the view keeps `engine.ViewVersion`.
