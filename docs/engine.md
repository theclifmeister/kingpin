...
- **Alerts** (`engine/alerts.go`). `Session.Alerts()` returns `[]Alert`, loudest first, in the order of the `AlertKind` constants: `talking`, `contract_due`, `debt_due`, `heat` (where you are, at or over the patrol line), `task_force`, `file` (#414: the DA's file two pages or fewer from the ones that indict you, `Count` its pages and `Amount` the indicting count, keyed by the pages so a fast-forward stops each time it grows; the TUI and the web client lead with `File 5/6: one more bust indicts you.` and say what adds pages and that lying low and the Legal upgrades take them off; the act is the dashboard, where the file is, the upgrades being no act's screen), `investigation` (#343: the open investigation, keyed by its city, target and opening night so a fast-forward stops the morning it opens; `city`, `target` corner | product | house with the `corner`, `product` or `house` id, `days` to the hit, 1 tonight), `no_corner` (#471, below), `front_shut` (#458: a front shut for upkeep the clean pile could not pay, while it is shut; `front` its id, `amount` what the pile lacked, `have` its upkeep, `days` to the reopening; keyed by the front and the reopening, the act the ledger), `float` (only while `FloatMatters`: a front, or a route with its dial on), `till` (#459: the wash has left the dirty pile at the till `[laundering] till_nights` nights running and the dial is not careful; `days` the nights, `amount` the till, `have` the dirty cash, `docs/laundering.md`), `wages`, `crew_line`, `skim`, `unposted`, `idle_corner`, `stash_full`, `scouts` (a faction on its way to a city where you earn, #341: `City`, `Level` the stage, `scouting` or `recruiting`, `Days` to the arrival; keyed by the faction, the city and the stage, so a fast-forward stops once on the scouts and once on the recruiting), `gate`, `house_known`, `da_race`, `retire`, `favour`, `reign` and `plan` (#347: the pinned plan's steps met in order, `docs/ambitions.md`).
...
- **Stops** (`engine/stops.go`).
  - `StopsOn(e)` decides whether an event stops a fast-forward, including each kind's conditions: an `Enforcement` past a patrol; a `CornerStruck` unless it is a war night that held; a `RivalBoosted` that failed; a `CornerTaken` from you; a `CornerLost` of yours, nobody working it (`idle`, #345) or the police's crackdown (#469: a corner of yours lost always stops, `TestLostCornersStop`); a `CrewShot` dead and yours; `PressureShifted` or `ReputationShifted` up; and a fixed list of kinds that always stop.
  - `Session.Stop(evs, before)` weighs a day in this order: a new stage, a card, an alert whose key the morning before lacked, the first event `StopsOn` names that the world says you could answer (`serves`, #442: a `ContractOffered` stops only in a city you stand in, work a corner in or hold stock in; one elsewhere is in the report and on the market and runs past, `TestOfferStopsWhereYouCanAnswerIt`) and that is news (`news`, #469, below).
  - **A trouble stops once** (#469). A playtest's F stopped every one to three days on a standing order short, a supply contract short of room and `Rosalind offers a tribute` (asked again every few days for five hundred days), while four corners and the crew went one by one among them. The session keeps a `memo` of the nights it ended (`remember`, called by `EndDay`; never saved, never read by a sim, no dice; `NewRun`, `Load`, `ImportSave` and `Attach` start it over, so a reload costs at most one stop more), and `news` filters `StopsOn` by it:
    - a `StandingShort` or a `SupplyShort` stops the morning it starts, not a morning the same order or contract (`shortKey`: the city and the product) was already short and uncovered the night before, the way an alert stops once by its key (`TestShortfallStopsOnce`: F 30 over a standing order short every night runs on to a real event);
...
- `TestEveryAlertHasAnAct` (#352): every kind names an act on a screen, and every alert eighty aggressive days raise carries one of its kind's. `TestUnpostedAlerts` and `TestStashFullAlerts` pin the two new kinds, their corner and city, their act and that each stops a fast-forward once.
...
That is 148 methods (#350 added `rules.heat.exposure_line`, #355 `rules.heat.rungs`), which makes 238 on the wire with the 90 methods above.
