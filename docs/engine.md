...
- **Alerts** (`engine/alerts.go`). `Session.Alerts()` returns `[]Alert`, loudest first, in the order of the `AlertKind` constants: `talking`, `contract_due`, `debt_due`, `heat` (where you are, at or over the patrol line), `task_force`, `investigation` (#343: the open investigation, keyed by its city, target and opening night so a fast-forward stops the morning it opens; `city`, `target` corner | product | house with the `corner`, `product` or `house` id, `days` to the hit, 1 tonight), `float` (only while `FloatMatters`: a front, or a route with its dial on), `wages`, `crew_line`, `skim`, `unposted`, `idle_corner`, `stash_full`, `gate`, `house_known`, `da_race`, `retire`, `favour` and `reign`.
  ...
  - **Every alert carries its act** (#352): `Alert.Act` is `{screen, mode, subject}`, what answers it. The engine names it and each front end maps it onto its own screens and dialogs; nothing new is simulated.
    `screen` is one of `dashboard`, `market`, `crew`, `map`, `ledger` (the TUI's tab words); `mode` is `post` (the post picker on the alert's `corner`) or empty; `subject` names the alert's own field that holds what the act opens on: `member`, `corner`, `contract`, `supplier`, `house` or `city`.
